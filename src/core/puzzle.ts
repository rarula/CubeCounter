...
export class Puzzle {
    ...
    private nextQuestion(size: Size): Question {
        ...
        const placedHeights: number[][] = [[size.height, ...Array<number>(size.depth - 1).fill(1)]];
        let placeableCoords: Coordinates[] = [];

        for (let x = 0; x < size.width - 1; x++) {
            placedHeights.push([1, ...Array<number>(size.depth - 1).fill(0)]);
        }
        ...
        const updatePlaceableCoords = (): void => {
            ...
            for (let x = 0; x < size.width; x++) {
                for (let z = 0; z < size.depth; z++) {
                    ...
                    const current = placedHeights[x][z];

                    // X Axis
                    if (0 < x) {
                        const top = placedHeights[x - 1][z];
                    ...
                    // Z Axis
                    if (0 < z) {
                        const left = placedHeights[x][z - 1];
                    ...
                    if ((0 < x && z == 0 && isPlaceableX) || (0 < z && x == 0 && isPlaceableZ) || (0 < x && 0 < z && isPlaceableX && isPlaceableZ)) {
                        placeableCoords.push({ x, y: current + 1, z });
                    }
                }
            }
        };
        ...
        for (let i = 0; i < count; i++) {
            ...
            const index = Math.abs(this.random.next()) % placeableCoords.length;
            const coords = placeableCoords[index];
            placedHeights[coords.x][coords.z]++;
        }
        ...
        return { scene: placedHeights, answer, choices };
    }
}
